(ns registrar.operation
  "OperationActor -- one higher-education operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (RegistrarOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Academic Integrity Governor
  ...
  One graph run = one higher-education operation (intake -> advise ->
  ...
  decision to a human operator (the registrar/academic dean). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:grade/finalize`/`:degree/confer` ALWAYS reach this node
  when the governor is clean -- see `registrar.phase`."
...
(defn build
  ...
  opts:
    :advisor      -- a `registrar.registraropsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (registraropsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; RegistrarOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (registraropsllm/-advise advisor store request)]
            {:proposal p :audit [(registraropsllm/trace request p)]})))

      ;; Academic Integrity Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
